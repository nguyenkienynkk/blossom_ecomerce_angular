...
export class UserProfileComponent implements OnInit {
  ...
  ngOnInit(): void {  
    ...
    this.userService.getUserDetail(this.token).subscribe({
      next: (response: any) => {
        ...
        const localDateOfBirth = this.convertToLocalDateAndAddOneDay(this.userResponse!.date_of_birth);
        this.userProfileForm.patchValue({
          fullname: this.userResponse?.fullname ?? '',
          address: this.userResponse?.address ?? '',
          date_of_birth: localDateOfBirth.toISOString().substring(0, 10),
        });
  ...
  convertToLocalDateAndAddOneDay(date: Date): Date {
    const localDate = new Date(date);
    localDate.setDate(localDate.getDate() + 1); // Cộng thêm 1 ngày
    return localDate;
  }
